// lib/features/ai/controller/admin_ai_refusals_controller.dart
//
// Step 19 — AI Refusals.
//
// Same shape as AiFailuresController, but:
//   - Calls listAiActivity with statusFilter: 'refused' (not 'error')
//   - Groups by refusalReason (with fallback to errorMessage, then "(no reason)")
//
// Refusals occur when the AI returns the refusal envelope. They count toward
// editorAiRefusalCount and trigger the 5-per-cycle soft block.
...
class AiRefusalsState {
  ...
  /// Resolve the best human-readable refusal label for one row.
  /// Prefers refusalReason → errorMessage → fallback.
  static String _labelFor(AiActivitySummary it) {
    final raw = (it.refusalReason ?? it.errorMessage ?? '').trim();
    if (raw.isEmpty) return '(no reason given)';
    return raw;
  }

  /// Group filtered refusals by their reason. Top groups first.
  /// Returns up to 10 entries.
  List<MapEntry<String, int>> get refusalGroups {
    final map = <String, int>{};
    for (final it in filteredItems) {
      var key = _labelFor(it);
...
class AiRefusalsController extends Notifier<AiRefusalsState> {
  @override
  AiRefusalsState build() {
    Future.microtask(refresh);
    return const AiRefusalsState.initial();
  }
...
final aiRefusalsProvider =
NotifierProvider<AiRefusalsController, AiRefusalsState>(
  AiRefusalsController.new,
);